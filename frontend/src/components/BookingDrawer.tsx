  ...
  useEffect(() => {
    const fetchSlots = async () => {
      ...
      try {
        ...
        const todayStr = formatDate(new Date()); // formatDate returns YYYY-MM-DD
        const futureDates = monthData
          .map((entry) => entry.date.slice(0, 10))
          .filter((dateStr) => dateStr >= todayStr); // only keep today & future
        ...
        const futureMonthData = monthData.filter(
          (entry) => entry.date.slice(0, 10) >= todayStr
        );

        if (!dateEntry && futureMonthData.length > 0) {
          const firstDate = new Date(futureMonthData[0].date);
          setSelectedDate(firstDate);
          return;
        }
